...
function createServer(bot, c) {
    ...
    app.post("/api/webhook/saweria", async (req, res) => {
        try {
            ...
            const notifMessage = 
                ...
                `\n\n${quote("Ingin membuat pesan spesial seperti ini? Yuk donasi melalui: https://saweria.co/RexbotX")}`;
    ...
    // Cek apakah SSL certificate tersedia
    try {
        const privateKey = fs.readFileSync('/etc/letsencrypt/live/vpsgcp.rendiichtiar.xyz/privkey.pem', 'utf8');
        const certificate = fs.readFileSync('/etc/letsencrypt/live/vpsgcp.rendiichtiar.xyz/cert.pem', 'utf8');
        const ca = fs.readFileSync('/etc/letsencrypt/live/vpsgcp.rendiichtiar.xyz/chain.pem', 'utf8');
        ...
        c.warn("SSL certificates not found, falling back to HTTP");
        return app;
    }
}
