...
module.exports = {
    ...
    code: async (ctx) => {
        ...
        if (!userDb?.premium) return await ctx.reply(quote("❎ User bukan pengguna premium!"));
        ...
        try {
            ...
            await ctx.sendMessage(userJid, {
                text: quote(`💡 Durasi premium Anda telah diubah oleh Owner!\n`) +
                    quote(`Durasi baru: ${duration} hari\n`) +
                    quote(`Berakhir pada: ${new Date(expiredTime).toLocaleString('id-ID')}`)
            });
            
            return await ctx.reply(quote(`✅ Berhasil mengubah durasi premium!\n`) +
